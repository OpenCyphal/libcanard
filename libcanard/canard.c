...
CANARD_PRIVATE struct cavl2_t* avlTrivialFactory(void* const user_reference)
{
    return (struct cavl2_t*) user_reference;
}
...
CANARD_PRIVATE int8_t txAVLPriorityPredicate(    //
    ...
    const struct cavl2_t* const node)
{
                                              ...
                                              const CanardMicrosecond            now_usec)
{
    ...
    size_t                    count   = 0;
    struct cavl2_t*           tx_node = cavl2_extremum(que->deadline_root, false);
...
struct CanardTxQueueItem* canardTxPeek(const struct CanardTxQueue* const que)
{
    ...
    if (que != NULL)
    {
        struct cavl2_t* const priority_node = cavl2_extremum(que->priority_root, false);
        ...
        ((frame->payload.data != NULL) || (0 == frame->payload.size)))
    {
        ...
        if (rxTryParseFrame(timestamp_usec, frame, &model))
        {
            if ((CANARD_NODE_ID_UNSET == model.destination_node_id) || (ins->node_id == model.destination_node_id))
            {
                ...
                struct cavl2_t* const sub_node = cavlSearch(&ins->rx_subscriptions[(size_t) model.transfer_kind],
                                                            &model.port_id,
                         ...
                         struct CanardRxSubscription* const out_subscription)
{
    ...
    if ((ins != NULL) && (out_subscription != NULL) && (tk < CANARD_NUM_TRANSFER_KINDS))
    {
        ...
        if (out >= 0)
        {
            ...
            const struct cavl2_t* const res = cavlSearch(&ins->rx_subscriptions[tk],
                           ...
                           const CanardPortID            port_id)
{
    ...
    if ((ins != NULL) && (tk < CANARD_NUM_TRANSFER_KINDS))
    {
        ...
        struct cavl2_t* const sub_node = cavlSearch(  //
